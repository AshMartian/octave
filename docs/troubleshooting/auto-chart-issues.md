...
1. Delete the project-local `.venv` (if present) if it is stale, then run Auto-Chart again. OCTAVE falls back to its isolated managed runtime when no local interpreter has STRUM's packages.
2. Or set `OCTAVE_STRUM_PYTHON` to the absolute path of a clean interpreter with `basic-pitch` installed.
