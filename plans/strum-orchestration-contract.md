...
The first proof is complete: OCTAVE created a 58-record allowed catalog; STRUM selected 31 Guitar, 52 Drums, 29 Bass, and 18 Keys-eligible records for dedicated workers. Catalog-aware smoke preprocessing and bounded CUDA training completed for Guitar, Drums, Bass, and Keys. Those artifacts validate the handoff and worker contracts only; they are not deployable quality models.
...
The catalog-aware Guitar, Drums, Bass, and Keys paths revalidate `allowed` catalog assets and create path-free task views. The first OCTAVE catalog proved 31 Guitar-eligible, 52 Drums-eligible, 29 Bass-eligible, and 18 Keys-eligible records. Guitar onset/fret, Drums onset-classifier, Bass onset/fret, and Keys onset/fret worker training are available for experiment artifacts. Bass and Keys are distinct `PART BASS` and `PART KEYS` five-lane tasks and emit distinct component/configuration identities; neither reuses Guitar task views or advertises inference capability. A Guitar pair can become an Expert-only neural profile only after revalidated validation-split evaluation passes explicit gates; an unqualified experiment remains non-deployable. A Drums V2 experiment can be packaged only as a hash-verified prepared-window evaluator, not as an auto-chart profile; direct V14 remains the executable Drums chart profile. Bass and Keys experiments explicitly require their own profile evaluation and packaging before either could become selectable. Five-lane chart-pair difficulty transforms are catalog-backed and worker-trainable for Guitar, Bass, Keys, and Drums. Vocals, Pro instruments, section, and mapper families also have catalog task views. Each declares its label-source/event schema and selected approved MIDI tracks, so a future trainer cannot silently infer track conventions; their missing learned architectures remain explicitly `planned` rather than falling back to raw folder scans.
...
**Implemented initial pipelines:** `guitar.onset-fret/v1` (Expert Guitar, prefer `audio.guitar`, fall back to `audio.mix`), `drums.onset-classifier/v1`, `bass.onset-fret/v1` (Expert `PART BASS`, prefer `audio.bass`, fall back to `audio.mix`), `keys.onset-fret/v1` (Expert `PART KEYS`, prefer `audio.keys`, fall back to `audio.mix`), and `chart_transform.five_lane/v1`. Prepare is a real catalog-backed job rather than a transition button.
...
The first templates are clearly labelled non-deployable Guitar, Drums, Bass, and Keys smoke runs, plus chart-transform fresh/fine-tune runs. STRUM must reject incompatible parents. A trained checkpoint is not an auto-chart profile unless it has a validated `model-bundle.json`, explicit inference capability, and evaluation evidence.
...
5. **Complete for worker artifacts:** catalog `prepare`/`train`, manifests, and compatibility checks for Guitar, Drums, Bass, Keys, and five-lane chart transforms. Bass and Keys are separate five-lane workers with explicit instrument-specific profile-evaluation/packaging gates. Guitar adds revalidated evaluation-gated Expert-profile packaging; Drums V2 adds evaluation-only package support, while deployable onset-plus-classifier execution remains a distinct future contract.
...
8. **Planned learned models:** Bass and Keys profile evaluation/packaging and deployable runtimes; Vocals, Pro instruments, mapper/section training architectures and deployable profiles on the unchanged catalog boundary.
