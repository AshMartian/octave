...
The catalog-aware Guitar, Drums, Bass, Keys, Vocal activity/pitch, Guitar/Bass fret-mapper, and Guitar/Bass section paths revalidate `allowed` catalog assets and create path-free task views. The first OCTAVE catalog proved 31 Guitar-eligible, 52 Drums-eligible, 29 Bass-eligible, 18 Keys-eligible, 3 Vocal-eligible, 31/29 Guitar/Bass mapper-eligible, and 30/29 usable Guitar/Bass section-eligible records. Section preparation excludes malformed third-party MIDI before publishing a task view, so readiness counts match the labeler rather than failing mid-job. Guitar onset/fret, Drums onset-classifier, Bass onset/fret, Keys onset/fret, Vocal activity/pitch, Guitar/Bass fret-mapper, and Guitar/Bass section worker training are available for experiment artifacts. Bass and Keys are distinct `PART BASS` and `PART KEYS` five-lane tasks and emit distinct component/configuration identities; neither reuses Guitar task views. Vocal activity/pitch is a distinct `PART VOCALS` frame task, deliberately excluding phrase, lyric, talky, harmony, and chart generation stages. Mapper training requires the STRUM `pitch` extra and otherwise fails closed before work begins; its catalog split and Basic Pitch provenance are now release-gated, and it has no deployable profile. Guitar, Bass, and Keys each have separate Expert-only neural profile packaging paths, but only after revalidated held-out evaluation passes explicit gates; no current smoke artifact is selected as a profile. A Drums V2 experiment can be packaged only as a hash-verified prepared-window evaluator, not as an auto-chart profile; direct V14 remains the executable Drums chart profile. Section artifacts retain catalog `source_id` lineage but are deliberately not router-compatible: their torchaudio HTK/reflect frontend differs from the legacy router's librosa Slaney/constant frontend. They require a matching frontend, tensor-only loader, calibration, chart-impact ablation, and instrument-specific composed profile before they could become selectable. Five-lane chart-pair difficulty transforms are catalog-backed and worker-trainable for Guitar, Bass, Keys, and Drums. Pro instruments remain catalog-task-ready with exact REAL-track schemas, a versioned planned-stage contract, and `execution: not_available`, not fabricated trainers.
...
**Implemented initial pipelines:** `guitar.onset-fret/v1` (Expert Guitar, prefer `audio.guitar`, fall back to `audio.mix`), `drums.onset-classifier/v1`, `bass.onset-fret/v1` (Expert `PART BASS`, prefer `audio.bass`, fall back to `audio.mix`), `keys.onset-fret/v1` (Expert `PART KEYS`, prefer `audio.keys`, fall back to `audio.mix`), `vocals.note-activity/v1` (Expert `PART VOCALS`, frame activity/pitch only), `strum.fret-mapper/guitar/v1`, `strum.fret-mapper/bass/v1` (each requires the `pitch` extra), and `chart_transform.five_lane/v1`. Prepare is a real catalog-backed job rather than a transition button.
