...
The catalog-aware Guitar, Drums, Bass, Keys, Vocal activity/pitch and phrase-boundary, Guitar/Bass fret-mapper, and Guitar/Bass section paths revalidate `allowed` catalog assets and create path-free task views. The first OCTAVE catalog proved 31 Guitar-eligible, 52 Drums-eligible, 29 Bass-eligible, 18 Keys-eligible, 3 Vocal-eligible, 31/29 Guitar/Bass mapper-eligible, and 30/29 usable Guitar/Bass section-eligible records. Section preparation excludes malformed third-party MIDI before publishing a task view, so readiness counts match the labeler rather than failing mid-job. Guitar onset/fret, Drums onset-classifier, Bass onset/fret, Keys onset/fret, Vocal activity/pitch and phrase boundaries, Guitar/Bass fret-mapper, and Guitar/Bass section worker training are available for experiment artifacts. Bass and Keys are distinct `PART BASS` and `PART KEYS` five-lane tasks and emit distinct component/configuration identities; neither reuses Guitar task views. The vocal workers are distinct exact-`PART VOCALS` frame and phrase tasks; lyric, talky, harmony, composition, evaluation, packaging, and chart execution remain explicit planned stages. Mapper training requires the STRUM `pitch` extra and otherwise fails closed before work begins; its catalog split and Basic Pitch provenance are now release-gated, and it has no deployable profile. Guitar, Bass, and Keys each have separate Expert-only neural profile packaging paths, but only after revalidated held-out evaluation passes explicit gates; no current smoke artifact is selected as a profile. A Drums V2 experiment can be packaged only as a hash-verified prepared-window evaluator, not as an auto-chart profile; direct V14 remains the executable Drums chart profile. Section artifacts retain catalog `source_id` lineage but are deliberately not router-compatible: their torchaudio HTK/reflect frontend differs from the legacy router's librosa Slaney/constant frontend. They require a matching frontend, tensor-only loader, calibration, chart-impact ablation, and instrument-specific composed profile before they could become selectable. Five-lane chart-pair difficulty transforms are catalog-backed and worker-trainable for Guitar, Bass, Keys, and Drums. Transform preparation can choose `audio_feature_mode: rms_onset_v1`; STRUM filters the immutable view to catalog audio that reaches the source Expert chart, records only source/asset hashes and roles, and fails closed rather than falling back to chart-only data. The first one-epoch CUDA smoke used 24 audio-aligned Guitar records (of 30 chart-pair candidates) and is an experiment only. Pro instruments remain catalog-task-ready with exact REAL-track schemas, a versioned planned-stage contract, and `execution: not_available`, not fabricated trainers.
...
**Implemented initial pipelines:** `guitar.onset-fret/v1` (Expert Guitar, prefer `audio.guitar`, fall back to `audio.mix`), `drums.onset-classifier/v1`, `bass.onset-fret/v1` (Expert `PART BASS`, prefer `audio.bass`, fall back to `audio.mix`), `keys.onset-fret/v1` (Expert `PART KEYS`, prefer `audio.keys`, fall back to `audio.mix`), `vocals.note-activity/v1` and `vocals.phrase-boundaries/v1` (exact `PART VOCALS` activity/pitch and phrase boundaries), `strum.fret-mapper/guitar/v1`, `strum.fret-mapper/bass/v1` (each requires the `pitch` extra), and `chart_transform.five_lane/v1` (chart-only or the catalog-audio-conditioned `rms_onset_v1` task mode). Prepare is a real catalog-backed job rather than a transition button.
...
The first templates are clearly labelled non-deployable Guitar, Drums, Bass, Keys, Vocal activity/pitch, and mapper smoke runs, plus chart-transform fresh/fine-tune runs. STRUM must reject incompatible parents. A trained checkpoint is not an auto-chart profile unless it has a validated `model-bundle.json`, explicit inference capability, and evaluation evidence.
...
5. **Complete for worker artifacts:** catalog `prepare`/`train`, manifests, and compatibility checks for Guitar, Drums, Bass, Keys, Vocal activity/pitch and phrase boundaries, Guitar/Bass fret mappers, Guitar/Bass section classifiers, and five-lane chart transforms. Chart transforms can make a catalog-audio-conditioned view and retain safe audio lineage; the first CUDA smoke is not a promoted quality result. Guitar, Bass, and Keys now have separate revalidated evaluation-gated Expert-profile packaging/runtime paths; current smoke artifacts have not passed promotion and are not packaged. Vocal and mapper bundles remain profile-gated; mappers declare the `pitch` runtime extra and release gates. Section bundles are explicitly profile-gated by a frontend/runtime mismatch, not silently adaptable. Drums V2 adds evaluation-only package support, while deployable onset-plus-classifier execution remains a distinct future contract.
6. **Complete for training; pending deployment UX:** OCTAVE dynamic Prepare/Train screens consume worker descriptors, readiness summaries, private request fields, and verified fine-tune-parent selection. Its Deploy screen must next consume bounded `checkpoint discover` results and map opaque artifact IDs to private main-process roots before validation; deployment remains gated on STRUM profile packaging/evaluation.
...
8. **Planned learned models:** held-out quality evaluation and promotion of the Guitar/Bass/Keys profile paths; the remaining Vocal phrase/lyric/talky/harmony stages and profile; mapper evaluation plus safe composed runtime; section matching-frontend calibration, ablation, and composed runtime; Pro-instrument training architectures and deployable profiles on the unchanged catalog boundary.
