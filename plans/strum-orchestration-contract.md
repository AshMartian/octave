...
The catalog-aware Guitar, Drums, Bass, Keys, Vocal activity/pitch, and Guitar/Bass fret-mapper paths revalidate `allowed` catalog assets and create path-free task views. The first OCTAVE catalog proved 31 Guitar-eligible, 52 Drums-eligible, 29 Bass-eligible, 18 Keys-eligible, 3 Vocal-eligible, and 31/29 Guitar/Bass mapper-eligible records. Guitar onset/fret, Drums onset-classifier, Bass onset/fret, Keys onset/fret, Vocal activity/pitch, and Guitar/Bass fret-mapper worker training are available for experiment artifacts. Bass and Keys are distinct `PART BASS` and `PART KEYS` five-lane tasks and emit distinct component/configuration identities; neither reuses Guitar task views or advertises inference capability. Vocal activity/pitch is a distinct `PART VOCALS` frame task, deliberately excluding phrase, lyric, talky, harmony, and chart generation stages. Mapper training requires the STRUM `pitch` extra and otherwise fails closed before work begins. A Guitar pair can become an Expert-only neural profile only after revalidated validation-split evaluation passes explicit gates; an unqualified experiment remains non-deployable. A Drums V2 experiment can be packaged only as a hash-verified prepared-window evaluator, not as an auto-chart profile; direct V14 remains the executable Drums chart profile. Bass, Keys, Vocals, and mappers explicitly require their own evaluation and packaging before they could become selectable. Five-lane chart-pair difficulty transforms are catalog-backed and worker-trainable for Guitar, Bass, Keys, and Drums. Pro instruments and section families remain catalog-task-ready with declared label schemas and explicit worker requirements, not fabricated trainers.
...
5. **Complete for worker artifacts:** catalog `prepare`/`train`, manifests, and compatibility checks for Guitar, Drums, Bass, Keys, Vocal activity/pitch, Guitar/Bass fret mappers, and five-lane chart transforms. Bass and Keys are separate five-lane workers with explicit instrument-specific profile-evaluation/packaging gates. Vocal and mapper bundles also remain profile-gated; mappers declare the `pitch` runtime extra. Guitar adds revalidated evaluation-gated Expert-profile packaging; Drums V2 adds evaluation-only package support, while deployable onset-plus-classifier execution remains a distinct future contract.
...
8. **Planned learned models:** Bass and Keys profile evaluation/packaging and deployable runtimes; the remaining Vocal phrase/lyric/talky/harmony stages and profile; mapper evaluation/packaging; Pro-instrument and section training architectures and deployable profiles on the unchanged catalog boundary.
