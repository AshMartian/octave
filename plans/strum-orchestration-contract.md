...
The first proof is complete: OCTAVE created a 58-record allowed catalog; STRUM selected 31 Guitar, 52 Drums, and 29 Bass-eligible records for their dedicated workers. Catalog-aware smoke preprocessing and bounded CUDA training completed for Guitar, Drums, and Bass. Those artifacts validate the handoff and worker contracts only; they are not deployable quality models.
...
The catalog-aware Guitar, Drums, and Bass paths revalidate `allowed` catalog assets and create path-free task views. The first OCTAVE catalog proved 31 Guitar-eligible, 52 Drums-eligible, and 29 Bass-eligible records. Guitar onset/fret, Drums onset-classifier, and Bass onset/fret worker training are available for experiment artifacts. Bass is a distinct `PART BASS`-only five-lane task and emits distinct Bass component/configuration identities; it does not reuse Guitar task views or advertise inference capability. A Guitar pair can become an Expert-only neural profile only after revalidated validation-split evaluation passes explicit gates; an unqualified experiment remains non-deployable. A Drums V2 experiment can be packaged only as a hash-verified prepared-window evaluator, not as an auto-chart profile; direct V14 remains the executable Drums chart profile. A Bass experiment explicitly requires Bass profile evaluation and packaging before it could become selectable. Five-lane chart-pair difficulty transforms are catalog-backed and worker-trainable for Guitar, Bass, Keys, and Drums. Keys, Vocals, Pro instruments, section, and mapper families also have catalog task views. Each declares its label-source/event schema and selected approved MIDI tracks, so a future trainer cannot silently infer track conventions; their missing learned architectures remain explicitly `planned` rather than falling back to raw folder scans.
...
The first templates are clearly labelled non-deployable Guitar, Drums, and Bass smoke runs, plus chart-transform fresh/fine-tune runs. STRUM must reject incompatible parents. A trained checkpoint is not an auto-chart profile unless it has a validated `model-bundle.json`, explicit inference capability, and evaluation evidence.
...
5. **Complete for worker artifacts:** catalog `prepare`/`train`, manifests, and compatibility checks for Guitar, Drums, Bass, and five-lane chart transforms. Bass is a separate `PART BASS` worker with an explicit profile-evaluation/packaging gate. Guitar adds revalidated evaluation-gated Expert-profile packaging; Drums V2 adds evaluation-only package support, while deployable onset-plus-classifier execution remains a distinct future contract.
...
8. **Planned learned models:** Bass profile evaluation/packaging and deployable runtime; Keys, Vocals, Pro instruments, mapper/section training architectures and deployable profiles on the unchanged catalog boundary.
