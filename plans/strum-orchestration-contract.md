...
The catalog-aware Guitar, Drums, Bass, Keys, Vocal activity/pitch, Guitar/Bass fret-mapper, and Guitar/Bass section paths revalidate `allowed` catalog assets and create path-free task views. The first OCTAVE catalog proved 31 Guitar-eligible, 52 Drums-eligible, 29 Bass-eligible, 18 Keys-eligible, 3 Vocal-eligible, and 31/29 Guitar/Bass mapper- and section-eligible records. Guitar onset/fret, Drums onset-classifier, Bass onset/fret, Keys onset/fret, Vocal activity/pitch, Guitar/Bass fret-mapper, and Guitar/Bass section worker training are available for experiment artifacts. Bass and Keys are distinct `PART BASS` and `PART KEYS` five-lane tasks and emit distinct component/configuration identities; neither reuses Guitar task views. Vocal activity/pitch is a distinct `PART VOCALS` frame task, deliberately excluding phrase, lyric, talky, harmony, and chart generation stages. Mapper training requires the STRUM `pitch` extra and otherwise fails closed before work begins. Guitar and Bass pairs can become distinct Expert-only neural profiles only after revalidated validation-split evaluation passes explicit gates; current Guitar/Bass smoke runs are below those gates and remain non-deployable. A Drums V2 experiment can be packaged only as a hash-verified prepared-window evaluator, not as an auto-chart profile; direct V14 remains the executable Drums chart profile. Keys, Vocals, mappers, and sections explicitly require their own evaluation, packaging, and runtime integration before they could become selectable. Five-lane chart-pair difficulty transforms are catalog-backed and worker-trainable for Guitar, Bass, Keys, and Drums. Pro instruments remain catalog-task-ready with exact REAL-track schemas, a versioned planned-stage contract, and `execution: not_available`, not fabricated trainers.
...
5. **Complete for worker artifacts:** catalog `prepare`/`train`, manifests, and compatibility checks for Guitar, Drums, Bass, Keys, Vocal activity/pitch, Guitar/Bass fret mappers, Guitar/Bass section classifiers, and five-lane chart transforms. Bass now has its own revalidated evaluation-gated Expert-profile packaging/runtime path; the current smoke evaluation fails its quality gate and is not packaged. Keys is a separate five-lane worker awaiting equivalent promotion work. Vocal, mapper, and section bundles remain profile-gated; mappers declare the `pitch` runtime extra. Guitar adds revalidated evaluation-gated Expert-profile packaging; Drums V2 adds evaluation-only package support, while deployable onset-plus-classifier execution remains a distinct future contract.
...
8. **Planned learned models:** Keys profile evaluation/packaging and deployable runtime; the remaining Vocal phrase/lyric/talky/harmony stages and profile; mapper and section evaluation/packaging/runtime integration; Pro-instrument training architectures and deployable profiles on the unchanged catalog boundary.
